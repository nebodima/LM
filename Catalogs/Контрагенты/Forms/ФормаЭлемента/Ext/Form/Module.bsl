...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		КонецПопытки;
		Попытка
			Объект.Родитель = Справочники.Контрагенты[Параметры.Ключ2.Группа];
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗаполнитьТЗДокументы()
	...
	Запрос.Текст = 
		...
		|	NULL,
		|	ПКО.Тариф,
		|	ПКО.ДокументОснования.ПредметОбучения,
		|	ПКО.Дата,
		|	NULL
		|ИЗ
...
Процедура ЗаполнитьДетьми()
	...
	Запрос.Текст = 
		...
		|	И Контрагенты.Родитель1 = &Контрагент";
