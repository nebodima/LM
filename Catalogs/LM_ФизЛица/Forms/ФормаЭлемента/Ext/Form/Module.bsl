...
Процедура ЗаполнитьИтоги()
	...
	Долги.Очистить();
	Элементы.ГруппаЗадолженность.Заголовок = "Задолженность по предметам";
	...
	Иначе 		
		...
		Если ТЗДолги.Количество()>0 Тогда
			Элементы.ГруппаЗадолженность.Заголовок = "Задолженность итого по предметам: " +Формат(Долги.Итог("Долг"),"ЧДЦ=2");
		КонецЕсли;
		...
		Если НЕ Объект.Тариф.Пустая() Тогда
			...
			Если ОстатокПоТарифу.Количество()>0 Тогда
				...
				Если Объект.Тариф.Сумма <> 0 Тогда
					...
					Если ОстатокДенегПоТарифу > 0 Тогда
						Элементы.ОстатокДенегПоТарифу.Заголовок = "Остаток денег";
						Элементы.ОстатокДенегПоТарифу.ЦветТекста =  WebЦвета.ТемноЗеленый;
						ОстатокЕдиницПоТарифу = ОстатокПоТарифу[0].СуммаКонечныйОстаток / ЦенаЗаЕдиницуТарифа;
					ИначеЕсли ОстатокДенегПоТарифу < 0 Тогда
						Элементы.ОстатокДенегПоТарифу.Заголовок = "Долг по тарифу";
						Элементы.ОстатокДенегПоТарифу.ЦветТекста =  WebЦвета.ТемноОранжевый;
						ОстатокДенегПоТарифу = -ОстатокДенегПоТарифу;
					КонецЕсли;
				КонецЕсли;
