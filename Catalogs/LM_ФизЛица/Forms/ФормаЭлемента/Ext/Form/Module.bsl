...
Процедура ЗаполнитьДиаграмму()
	...
	Иначе
		...
		Сч = 0;
		Для Каждого стр из Долги Цикл
			Сч = Сч + 1;
			Декорация1 = Элементы.Добавить("ЗадолженностьПредмет"+Сч, Тип("ДекорацияФормы"),Элементы.ГруппаЗадолженность);
			Декорация1.Заголовок = "" + ?(НЕ ЗначениеЗаполнено(стр.Предмет),"<предмет не выбран>",стр.Предмет) + ":  " +Формат(стр.Долг,"ЧДЦ=2");
		КонецЦикла;
		Если Долги.Количество()>1 Тогда
			Декорация1 = Элементы.Добавить("ЗадолженностьПредмет"+Сч+1, Тип("ДекорацияФормы"),Элементы.ГруппаЗадолженность);
			Декорация1.Заголовок = "Итого:  " +Формат(Долги.Итог("Долг"),"ЧДЦ=2");
			Декорация1.Шрифт = Новый Шрифт(, , 10, Истина);
			Декорация1.Гиперссылка = Истина;
			Декорация1.УстановитьДействие("Нажатие","ОткрытьВзаиморасчетыНаКлиенте2");
		КонецЕсли;
