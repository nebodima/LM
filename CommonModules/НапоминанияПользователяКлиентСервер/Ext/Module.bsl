...
Функция ПредставлениеВремени(Знач Время, ПолноеПредставление = Истина, ВыводитьСекунды = Истина) Экспорт
	...
	//DEDa (
	КоличествоЛет = Цел(Время / 60/60/24/365);
	//DEDa )
	...
	//DEDa (
	КоличествоЛет   = КоличествоДней - КоличествоЛет * 365;
	//DEDa )
	...
	ИначеЕсли КоличествоЛет > 0 И КоличествоДней+КоличествоЧасов+КоличествоМинут+КоличествоСекунд=0 Тогда
		Результат = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоЛет, ПредставлениеЛет);
	Иначе
...
Функция ЗаменитьЕдиницуИзмеренияНаМножитель(Знач Единица)
	...
	Если ПервыеТриСимвола = НСтр("ru = 'нед'") Или ПервыеТриСимвола = НСтр("ru = 'н'") Тогда
		...
		//DEDa (
	ИначеЕсли ПервыеТриСимвола = НСтр("ru = 'год'") Или ПервыеТриСимвола = НСтр("ru = 'г'") Тогда
		Результат = 60*60*24*365;
		//DEDa )
