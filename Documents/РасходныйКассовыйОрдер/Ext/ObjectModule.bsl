...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВидОперации = Перечисления.LM_ВидыОпераций.КассаУченики Тогда
		...
		Если ЗначениеЗаполнено(ДокументОснования) Тогда
			...
			СтрокаУченика = ДокументОснования.Ученики.Найти(Контрагент,"Ученик");
			Если ЗначениеЗаполнено(СтрокаУченика) Тогда
				Движение.Тариф = СтрокаУченика.Тариф;
				Движение.Часы  = -('00010101' - СтрокаУченика.Часы);
			КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбменДанными.Загрузка Тогда
		ДокументКПроведению = Справочники.ДокументыКПроведению.СоздатьЭлемент();
		ДокументКПроведению.Дата = ТекущаяДата();
		
		//Если ЭтотОбъект.ЭтоНовый() Тогда 
		//	Если НЕ ЗначениеЗаполнено(Ссылка) Тогда 
		//		// Если ссылка нового пустая, то запрашиваем ее у менеджера и устанавливаем 
		//		СсылкаНаОбъект = Документы.ПКО.ПолучитьСсылку(); 
		...
		Если Проведен Тогда
			ДокументКПроведению.Документ = СсылкаНаОбъект;
			ДокументКПроведению.Действие = "Проведение";			
		ИначеЕсли ПометкаУдаления Тогда
			ДокументКПроведению.Документ = СсылкаНаОбъект;
			ДокументКПроведению.Действие = "Удаление";
		ИначеЕсли НЕ Проведен Тогда
			ДокументКПроведению.Документ = СсылкаНаОбъект;
			ДокументКПроведению.Действие = "ОтменаПроведения";			
		КонецЕсли;
		ДокументКПроведению.Записать();
