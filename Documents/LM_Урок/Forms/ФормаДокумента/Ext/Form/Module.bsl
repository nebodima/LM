...
Функция РассчитатьЗадолженность(Ученик, ПредметОбучения)
	...
	Иначе
		...
		|	СписанияСуммЗаУрокиОстатки.СуммаОстаток КАК СуммаОстаток
		|ИЗ
		|	РегистрНакопления.LM_СписанияСуммЗаУроки.Остатки(, Клиент = &Клиент) КАК СписанияСуммЗаУрокиОстатки";   	
	КонецЕсли;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		Для Каждого Стр из Объект.Ученики Цикл
			...
			Стр.ЗадолженностьОбщая		= РассчитатьЗадолженность(Стр.Ученик, Неопределено);
			Стр.ОстатокУроковЧасов		= РассчитатьОстатокУроковЧасов(Стр.Ученик, Стр.Тариф, Стр.ЗадолженностьПоПредмету);
