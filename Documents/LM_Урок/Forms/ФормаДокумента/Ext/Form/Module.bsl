...
Процедура ОбновитьВидимостьКратко()
	...
	Для Каждого Стр из Объект.Ученики Цикл
		...
		Стр.ЗадолженностьОбщая		= РассчитатьЗадолженность(Стр.Ученик, Неопределено, Объект.Организация);
		Стр.ОстатокБонусов 			= РассчитатьОстатокБонусов(Стр.Ученик, Стр.ДисконтнаяКарта, Объект.Организация);
		Если ЗначениеЗаполнено(Стр.НачислениеПедагогу) Тогда
	...
	Если КоличествоНачислений>0 Тогда
		Элементы.ГруппаНачисленияПедагогу.Заголовок = ""+КоличествоНачислений+ " начислений педагогу (" +Формат(Объект.Ученики.Итог("НачислениеПедагогу"),"ЧДЦ=2")+")";
	КонецЕсли;
...
Процедура ПересчитатьНачисления(Команда)
	ПересчитатьНачисленияНаСервере();
КонецПроцедуры
Процедура ПересчитатьНачисленияНаСервере()
	
	Если ЗначениеЗаполнено(Объект.СтавкаПедагога) Тогда
		Для Каждого стр из Объект.Ученики Цикл
			
			Если Стр.НеИзменять Тогда
				Продолжить;
			КонецЕсли;
			
			
			Если Объект.СтавкаПедагога.ОплатаЗаЗанятие>0 Тогда
				  Стр.НачислениеПедагогу = Объект.СтавкаПедагога.ОплатаЗаЗанятие;
			  ИначеЕсли Объект.СтавкаПедагога.ОплатаЗаЧас>0 Тогда
				  КолЧасов = -('00010101' - Стр.Часы) / 60 / 60;
				  Стр.НачислениеПедагогу = Объект.СтавкаПедагога.ОплатаЗаЧас * КолЧасов;
			ИначеЕсли Объект.СтавкаПедагога.ОплатаСЧеловека>0 Тогда
				 Стр.НачислениеПедагогу = Объект.СтавкаПедагога.ОплатаСЧеловека;
			ИначеЕсли Объект.СтавкаПедагога.ПроцентСОплатыУченика>0 Тогда
				 Стр.НачислениеПедагогу = Стр.Сумма * Объект.СтавкаПедагога.ПроцентСОплатыУченика / 100;
			 КонецЕсли;
