...
Процедура ОбновитьВидимость()
		...
		Иначе				
			...
			Если Стр.СписатьОплату Тогда
				...
				Если ЗначениеЗаполнено(Стр.Коэффициент) Тогда
					...
					Стр.СуммаБезСкидки 	= СуммаПоТарифуЗаЧасБезСкидки * КоэффициентСекунд;
					Стр.Скидка 			= Стр.СуммаБезСкидки - Стр.Сумма;				
	...
	Если ЗначениеЗаполнено(Объект.Организация) и Объект.Организация.LM_ИспользоватьОкругление Тогда 
		Объект.СуммаСкидки 	= Окр(Объект.Ученики.Итог("Скидка"));
	Иначе
...
Процедура ШаблонОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ШаблонОбработкаВыбораНаСервере(ВыбранноеЗначение);
КонецПроцедуры
Процедура ШаблонОбработкаВыбораНаСервере(Шаблон)
	
	Если ЗначениеЗаполнено(Шаблон) Тогда
		
		Объект.Дата 			= НачалоДня(ТекущаяДата())-('00010101'-Шаблон.ПоУмолчаниюВремяНачалаУрока);
		Объект.Педагог 			= Шаблон.ПоУмолчаниюФизЛицо;
		Объект.ПредметОбучения 	= Шаблон.ПоУмолчаниюПредмет;
		Объект.ГруппаОбучения 	= Шаблон.ПоУмолчаниюГруппаОбучения;
		Объект.КоличествоЧасов 	= Шаблон.ПоУмолчаниюПродолжительностьУрока;
		Объект.Помещение 		= Шаблон.ПоУмолчаниюПомещение;
