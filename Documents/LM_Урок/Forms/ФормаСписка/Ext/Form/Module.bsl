...
Процедура ПостроитьДиаграмму()
	...
	//	Запрос.Текст = СтрЗаменить(Запрос.Текст,"И СписанияСуммЗаУроки.Клиент.Родитель В иерархии(&Группа)","");
	//Иначе
	//	Запрос.УстановитьПараметр("Группа", Справочники.LM_ФизЛица.Ученики);
	//КонецЕсли;
...
Функция СоздатьПКОНаОснованииНаСервере()
	...
	ИначеЕсли ЗначениеЗаполнено(Элементы.Список.ТекущаяСтрока) Тогда
		ЗначенияЗаполнения.Вставить("ДокументОснования", Элементы.Список.ТекущаяСтрока);
	КонецЕсли;
