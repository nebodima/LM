		...
		Для Каждого Стр из Элементы.Календарь.ВыделенныеДаты Цикл
			...
			//Если ЗначениеЗаполнено(ГруппаОбучения) Тогда
				...
				Если ВыборкаДетальныеЗаписи.Количество() = 0 Тогда
					...
					Если ВзятьВремяИзГруппыОбучения Тогда
...
Процедура ГруппаОбученияПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.ГруппаОбучения) Тогда
		...
		Если ДобавлениеВСуществующиеЗанятия Тогда
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	КонтрагентыГруппыОбучения.Ссылка КАК Ссылка
			|ИЗ
			|	Справочник.Контрагенты.ГруппыОбучения КАК КонтрагентыГруппыОбучения
			|ГДЕ
			|	КонтрагентыГруппыОбучения.Ссылка.ТолькоВыбранныеГруппыОбучения
			|	И КонтрагентыГруппыОбучения.ГруппаОбучения = &ГруппаОбучения";	
			Запрос.УстановитьПараметр("ГруппаОбучения", ГруппаОбучения);	
			РезультатЗапроса = Запрос.Выполнить(); 	
			ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
...
Процедура ОбновитьВидимость()
	...
	Если ЗаполнятьИзГруппыОбучения Тогда 
		...
		Элементы.ВремяНачала.Доступность 		= Ложь;
		Элементы.КоличествоЧасов.Доступность 	= Ложь;
		Элементы.ВзятьВремяИзГруппыОбучения.Доступность 	= Ложь;
		ВзятьВремяИзГруппыОбучения 				= Ложь;
	Иначе
		Элементы.Помещение.Доступность 			= Истина;
		Элементы.Педагог.Доступность 			= Истина;
		Элементы.ПредметОбучения.Доступность 	= Истина;
		Элементы.ВзятьВремяИзГруппыОбучения.Доступность 	= Истина;
	КонецЕсли;
...
Процедура УченикиУченикСоздание(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Контрагенты.Форма.ФормаЭлемента",Пар,ЭтотОбъект,УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НайтиДокументыНаСервере()
	...
	//Если ДобавлениеВСуществующиеЗанятия Тогда
		...
		Для Каждого Стр из Элементы.Календарь.ВыделенныеДаты Цикл
			...
			Если ЗаполнятьИзГруппыОбучения или ВзятьВремяИзГруппыОбучения Тогда
				ВремяЗанятия 	= ПолучитьВремяИзГруппыОбучения(Стр);
				КолЧасов 		= ГруппаОбучения.Продолжительность;
			КонецЕсли;
...
&НаКлиенте
Процедура ВзятьВремяИзГруппыОбученияПриИзменении(Элемент)	
	ОбновитьВидимость();    	
КонецПроцедуры
