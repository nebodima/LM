Функция ПолучитьВремяИзГруппы(Стр)
	...
	Если ДеньНедели = "Пн" Тогда
		ВремяНачала = ?(ГруппаОбучения.ПН,ГруппаОбучения.ПНВремя,ГруппаОбучения.Время);
		ВремяОкончания = ?(ГруппаОбучения.ПН,ГруппаОбучения.ПНВремя,ГруппаОбучения.Время);
		...
		Для Каждого Стр из Элементы.Календарь.ВыделенныеДаты Цикл
			...
			//Если ЗначениеЗаполнено(ГруппаОбучения) Тогда
				...
				Если ЗаполнятьИзГруппыОбучения Тогда
					...
					Запрос.УстановитьПараметр("ДатаНачала", Стр-('00010101'-ГруппаОбучения.Время)); 
					Запрос.УстановитьПараметр("ДатаОкончания", Стр-('00010101'-ГруппаОбучения.Время)-('00010101'-ГруппаОбучения.Продолжительность));
				Иначе
					//Запрос.УстановитьПараметр("ГруппаОбучения", ГруппаОбучения);
