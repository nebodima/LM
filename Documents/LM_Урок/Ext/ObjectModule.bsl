...
Функция ПроверитьАбонемент(Стр)
	...
	Иначе
		...
		Если Стр.Абонемент.ДатаНачала > НачалоДня(Ссылка.Дата) Тогда
			Возврат "У ученика " +Стр.Ученик+ " дата начала абонемента больше даты урока! Строка: " +Стр.НомерСтроки;
		ИначеЕсли Стр.Абонемент.ДатаОкончания < КонецДня(Ссылка.Дата) Тогда
		...
		Для Каждого СтрПриостановка из Стр.Абонемент.Приостановки Цикл 			
			Если НачалоДня(Ссылка.Дата) > СтрПриостановка.ДатаПриостановки и КонецДня(Ссылка.Дата) < СтрПриостановка.ДатаВозобновления Тогда
