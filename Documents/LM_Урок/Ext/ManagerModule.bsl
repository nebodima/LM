...
Процедура ПересчитатьНачисленияПедагога(Объект, ПолныйПересчет) Экспорт
	...
	Для Каждого СтрУч из Объект.Ученики Цикл
		...
		Если НЕ ЗначениеЗаполнено(Ставка) или СтрУч.СписатьОплату = Ложь Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли Ставка.ОплатаСЧеловека > 0 Тогда
...
Процедура РассчитатьТабличнуюЧасть(Объект) Экспорт
	...
	Для Каждого Стр из Объект.Ученики Цикл				
		...
		Если ЗначениеЗаполнено(Стр.Тариф) и Стр.СписатьОплату Тогда 
			...
			Стр.Сумма = СтруктураЦеныТарифа.Сумма / СтруктураЦеныТарифа.КоличествоЧасов;
