...
Процедура ПересчитатьНачисленияПедагога(Объект, ПолныйПересчет) Экспорт
	        	
	//Если Объект.Ссылка.Пустая() Тогда
	//	СсылкаНового = Документы.LM_Урок.ПолучитьСсылку();
	//	ЭтотОбъект.УстановитьСсылкуНового(СсылкаНового);   ЭтотОбъект.у
	//	СсылкаНаДокумент = ЭтотОбъект.ПолучитьСсылкуНового();  Объект.устан
	//Иначе
	//	СсылкаНаДокумент = Объект;
	//КонецЕсли;
	
	// Отбираем только доп начисления (не учеников)
	Отбор = Новый Структура();
	Отбор.Вставить("Ученик",Справочники.LM_ФизЛица.ПустаяСсылка());
	ТЗ = Объект.НачисленияПедагогу.Выгрузить();
	...
	Для Каждого СтрУч из Объект.Ученики Цикл
		...
		//Если СтрУч.Явка или СтрУч.СписатьОплату Тогда
		//	МожноНачислять = Истина;
		//КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Ставка) или СтрУч.СписатьОплату = Ложь
			или Ставка.ОплатаЗаЗанятие > 0 или Ставка.ОплатаЗаЧас > 0 Тогда
			Продолжить;
		КонецЕсли;   		
				
		НовНачисление = Объект.НачисленияПедагогу.Добавить();
		НовНачисление.Ученик = СтрУч.Ученик;
		
		Если Ставка.ОплатаСЧеловека > 0 Тогда
			НовНачисление.СуммаНачисления = Ставка.ОплатаСЧеловека;
		ИначеЕсли Ставка.ПроцентСОплатыУченика > 0 Тогда
			НовНачисление.СуммаНачисления = СтрУч.Сумма * Ставка.ПроцентСОплатыУченика / 100;
		КонецЕсли; 			
		
	КонецЦикла;
	
	//Если МожноНачислять Тогда
		Если Ставка.ОплатаЗаЗанятие > 0 Тогда
			НовНачисление = Объект.НачисленияПедагогу.Добавить();
	...
	// Возвращаем начисление НеУчеников
	Для Каждого СтрокаСтаройТЗ из НайденныеСтроки Цикл		
		Если СтрНайти(СтрокаСтаройТЗ.Комментарий,"#")>0 Тогда
			Продолжить;
		КонецЕсли;
