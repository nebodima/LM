...
Процедура КлиентПриИзмененииНаСервере()
	...
	Попытка
		Если ЗначениеЗаполнено(Объект.Клиент.Родитель1) Тогда
			Объект.Родитель = Объект.Клиент.Родитель1;
		КонецЕсли;
	Исключение
	КонецПопытки;
...
Функция ОпределитьСписокУроков(СписокУроков)
	...
	Запрос.УстановитьПараметр("Ученик", Объект.Клиент);
...
Процедура СформироватьТекстНазначенияПлатежа(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Клиент) Тогда
		Объект.НазначениеПлатежа = "Оплата за ученика: " +Объект.Клиент+ 
...
Процедура ДисконтнаяКартаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыОтбора = Новый Структура("Владелец", Объект.Клиент); 
...
Процедура ПолучитьСуммуПоТарифу(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Тариф) Тогда		
		Объект.Сумма = ПолучитьСуммуПоТарифуНаСервере();
	КонецЕсли;
