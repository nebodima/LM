...
Процедура ЗаполнитьНаСервере(ДатаНачала,ДатаОкончания)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	LM_УрокУченики.Ссылка КАК Ссылка,
		|	LM_Урок.Дата КАК Дата,
		|	LM_Урок.КоличествоЧасов КАК КоличествоЧасов,
		|	LM_Урок.Педагог КАК Педагог,
		|	LM_Урок.Номер КАК Номер,
		|	LM_Урок.Помещение КАК Помещение
		|ИЗ
		|	Документ.LM_Урок.Ученики КАК LM_УрокУченики
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.LM_Урок КАК LM_Урок
		|		ПО LM_УрокУченики.Ссылка = LM_Урок.Ссылка
		|ГДЕ
		|	LM_УрокУченики.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И LM_УрокУченики.Ссылка.ГруппаОбучения = &ГруппаОбучения
		|	И LM_УрокУченики.Ссылка.Педагог = &Педагог
		|	И LM_УрокУченики.Ссылка.Помещение = &Помещение
		|	И LM_УрокУченики.Ученик = &Ученик
		|	И LM_УрокУченики.Ссылка.Проведен";
	
	Если НЕ ЗначениеЗаполнено(ФильтрПедагог) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И LM_УрокУченики.Ссылка.Педагог = &Педагог","");
	КонецЕсли;              	
	
	Если НЕ ЗначениеЗаполнено(ФильтрПомещение) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И LM_УрокУченики.Ссылка.Помещение = &Помещение","");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ФильтрГруппаОбучения) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И LM_УрокУченики.Ссылка.ГруппаОбучения = &ГруппаОбучения","");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ФильтрУченик) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И LM_УрокУченики.Ученик = &Ученик","");
	КонецЕсли;
