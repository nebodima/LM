...
Процедура ЗаполнитьНаСервере(ДатаНачала,ДатаОкончания)
	...
	Если ФильтрУченик.Пустая() Тогда
		...
		|	И LM_Урок.Ссылка.Статус = &Статус
		|	И LM_Урок.Ссылка.ГруппаОбучения = &ГруппаОбучения
		|	И LM_Урок.Ссылка.Проведен = ИСТИНА
		|
		|УПОРЯДОЧИТЬ ПО
		|	Помещение,
		|	Педагог
	...
	Иначе
		...
		|	LM_Урок.Ссылка.Статус КАК Статус,
	...
	Если НЕ ЗначениеЗаполнено(СтатусУрока) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И LM_Урок.Ссылка.Статус = &Статус","");
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Статус", СтатусУрока);
	РезультатЗапроса = Запрос.Выполнить(); 	
...
Функция ДействиеСДокументом()
	...
	Если ТекДанные.Количество() = 1 Тогда
		...
		ПомОткуда 	= ТекДанные[0].Значение.Помещение;
		ПомКуда 	= ТекДанные[0].ЗначенияИзмерений.Получить("Помещение");
		
		Если ТекДанные[0].ЗначенияИзмерений.Количество() = 1 Тогда
			
			// Это смена помещения
			Если ПомОткуда <> ПомКуда Тогда
				
				Если ТекДанные[0].Значение.Дата <> ТекДанные[0].Начало // в том числе перенос по времени
					или ТекДанные[0].Значение.Окончание <> ТекДанные[0].Конец Тогда
					С.Вставить("Действие","Сменить помещение " +ПомОткуда+ " на " +ПомКуда+ Символы.ПС 
				...
				Иначе // Только смена помещения 
					С.Вставить("Действие","Сменить помещение " +ПомОткуда+ " на " +ПомКуда+ " ?");
				КонецЕсли;
				
			// Это перенос по времени
			ИначеЕсли ТекДанные[0].Значение.Дата <> ТекДанные[0].Начало
				или ТекДанные[0].Значение.Окончание <> ТекДанные[0].Конец Тогда
			...
			// Это смена Помещения
			//Если ПомОткуда <> ПомКуда Тогда
			//	С.Вставить("Действие","Сменить помещение " +ПомОткуда+ " на " +ПомКуда+ Символы.ПС +ТекДанные[0].Значение+ " ?");
			//	С.Вставить("Помещение",ПомКуда);
		...
		Иначе
			...
			Если ТекДанные[0].Значение.Дата <> ТекДанные[0].Начало
				...
				+Формат(ТекДанные[0].Значение.Дата,"ДФ='дд.ММ.гг (ЧЧ:мм)'")+ " - " +Формат(ТекДанные[0].Значение.Окончание,"ДФ='дд.ММ.гг (ЧЧ:мм)'")+ " на " +Символы.ПС
				+Формат(ТекДанные[0].Начало,"ДФ='дд.ММ.гг (ЧЧ:мм)'")+ " - " +Формат(ТекДанные[0].Конец,"ДФ='дд.ММ.гг (ЧЧ:мм)'")+ Символы.ПС +ТекДанные[0].Значение+ " ?");
				С.Вставить("Помещение",ПомОткуда);
				С.Вставить("Дата",ТекДанные[0].Начало);
				С.Вставить("Окончание",ТекДанные[0].Конец);
				С.Вставить("Документ",ТекДанные[0].Значение);
				Возврат С;
...
&НаКлиенте
Процедура РазделительКартинка1Нажатие(Элемент)
	Элементы.ГруппаЛево.Видимость = НЕ Элементы.ГруппаЛево.Видимость;
	Элементы.РазделительКартинка1.Видимость = Элементы.ГруппаЛево.Видимость;
	Элементы.РазделительКартинка2.Видимость = НЕ Элементы.ГруппаЛево.Видимость;
КонецПроцедуры
