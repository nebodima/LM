...
Функция ОформитьПКОНаСервере()
	...
	Попытка
		Если Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].Использование Тогда 	 
			ЗначенияЗаполнения.Вставить("LM_ФизЛицо", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].ПравоеЗначение);
			ЗначенияЗаполнения.Вставить("ПредметОбучения", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[2].ПравоеЗначение);
			ЗначенияЗаполнения.Вставить("Родитель", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].ПравоеЗначение.Родитель1);
			ЗначенияЗаполнения.Вставить("СуммаДокумента", -Число(элементы.ОтчетТабличныйДокумент.ТекущаяОбласть.Текст));
		КонецЕсли;
	Исключение
	КонецПопытки;
...
&НаКлиенте
Процедура ОформитьПКО(Команда)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыОткрытия = ОформитьПКОНаСервере();
...
&НаСервере
Функция ОформитьПеремещениеДенегНаСервере()
	...
	Попытка
		Если Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].Использование Тогда 	 
			ЗначенияЗаполнения.Вставить("Клиент", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].ПравоеЗначение);
			ЗначенияЗаполнения.Вставить("ПредметОбученияОтправитель", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[2].ПравоеЗначение);
			ЗначенияЗаполнения.Вставить("Сумма", -Число(элементы.ОтчетТабличныйДокумент.ТекущаяОбласть.Текст));
		КонецЕсли;
	Исключение
	КонецПопытки;
...
&НаКлиенте
Процедура ОформитьПеремещениеДенег(Команда)
	СтандартнаяОбработка = Ложь;
	
	ПараметрыОткрытия = ОформитьПеремещениеДенегНаСервере();
	ОткрытьФорму("Документ.LM_ПеремещениеДенег.ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма);
КонецПроцедуры
